...
class AppSidebarNav2 extends Component {
  ...
  handleClick(e, item) {
    if (
      item.attributes &&
      typeof item.attributes.onClick === "function" &&
      !this.isActiveRoute(item.url, this.props)
    ) {
  ...
  navIcon(item) {
    ...
    const iconObject =
      typeof icon === "object" && icon !== null
        ? { iconClass: icon.class, iconClassName: icon.className, ...icon }
        : { iconClass: icon };
    ...
    const iconComponent = iconImg ? (
      <img {...iconAttr} className={classIcon} src={iconImg.src} alt="" />
    ) : (
      <i {...iconAttr} className={classIcon}>
        {iconInner}
      </i>
    );
  ...
  isExternal(url, props) {
    ...
    const link =
      linkType === "string"
        ...
        ? url.pathname
        : linkType === "function" && typeof url(props.location) === "string"
        ? url(props.location)
        : linkType === "function" && typeof url(props.location) === "object"
    ...
    if (window.MutationObserver) {
      ...
      this.changes = new MutationObserver((mutations) => {
        const isSidebarMinimized =
          document.body.classList.contains("sidebar-minimized") || false;
  ...
  componentWillUnmount() {
    try {
      ...
      // eslint-disable-next-line
      console.warn(
        "CoreUI SidebarNav failed to disconnect from MutationObserver",
        ignore
      );
    }
  }
  ...
  render() {
    ...
    const options = Object.assign(
      {},
      { suppressScrollX: true, suppressScrollY: this.state.sidebarMinimized }
    );
